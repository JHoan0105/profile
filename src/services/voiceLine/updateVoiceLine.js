...
// Default function
export default async function updateVoiceLine(voiceLine) {
  const controller = new AbortController();
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  console.log("updateVoiceLine(id) SalesforceID", voiceLine.id)
  try {
    const verify = await axios.put(`${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/provisioning/certus/templates/voiceline/${voiceLine.id}`,
      {
        "id": voiceLine.id,
        "voiceServiceTemplate": voiceLine.voiceServiceTemplate, // Assigned voice service template

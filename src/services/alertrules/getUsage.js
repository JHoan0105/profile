...
// Default function
export default async function getUsage(accountNumber, dateFrom, dateTo) {
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  const route = `${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/flexbill/account/${accountNumber}/usage?dateFrom=${dateFrom}&dateTo=${dateTo}`;
  ...
  try {
    ...
    console.log("AlertRulesList:", response.data);

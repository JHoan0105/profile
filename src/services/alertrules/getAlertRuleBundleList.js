...
// Default function
export default async function getAlertRuleBundleList(accountNumber, relativeThreshold, thresholds) {
  const controller = new AbortController();
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  const route = `${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/alertrulebundlelist`;
  ...
  try {
    ...
    console.log("AlertRuleBundleList:", response.data);

...
// Default function
export default async function getAlertRules(accountNumber, groupId) {
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  const route = `${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/flexbill/account/${accountNumber}/alert/group/${groupId}/rules`;

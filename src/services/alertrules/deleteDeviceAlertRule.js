...
// Default function
export default async function deleteDeviceAlertRule(accountNumber, groupId, id) {
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  console.log('Delete Alert Group', {
    "accountNumber": accountNumber,
    "id": id,
    "groupId": groupId
  })
  try {
    const verify = await axios.delete(`${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/flexbill/account/${accountNumber}/alert/group/${groupId}/rule/${id}`,
      {
        headers: {

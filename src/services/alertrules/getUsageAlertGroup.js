...
// Default function
export default async function getUsageAlertGroup(accountNumber) {
  const controller = new AbortController();
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  const route = `${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/flexbill/account/${accountNumber}/alert/group`;
  ...
  try {
    ...
    console.log("UsageAlertGroupEmailList:", response.data);

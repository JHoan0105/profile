...
// Default function
export default async function getVoiceServiceListByAccountNumber(accountNumber) {
  const controller = new AbortController();
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  const route = `${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/provisioning/account/${accountNumber}/certus/templates/voice`;
  ...
  try {
    ...
    console.log("voiceServiceList: ", response.data);

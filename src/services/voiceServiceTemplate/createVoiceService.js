...
// Default function
export default async function createVoiceService(voiceService) {
  console.log("createVoiceService");
  const controller = new AbortController();
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));

  try {
    const verify = await axios.post(`${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/provisioning/certus/templates/voice`,
      voiceService,
      {

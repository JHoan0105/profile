...
// Default function
export default async function getDataServiceAssignments(id) {
    const controller = new AbortController();
    const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
    const route = `${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/provisioning/certus/templates/${id}/assignments`;
  try {
    console.log(route);

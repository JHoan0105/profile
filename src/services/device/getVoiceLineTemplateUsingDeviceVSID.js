...
// Default function
export default async function getVoiceLineTemplateUsingDeviceVSID(accountNumber, servicePlanID) {
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  console.log("Account Number", accountNumber, servicePlanID)
  try {
    const verify = await axios.get(`${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/provisioning/account/${accountNumber}/certus/bundle/${servicePlanID}`,
      {
        headers: {

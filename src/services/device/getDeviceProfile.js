...
// Default function
export default async function getDeviceProfile(a, i) {
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  console.log("Device IMEI", i)
  try {
    const verify = await axios.get(`${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/provisioning/account/${a}/certus/device/${i}/profile`,
      {
        headers: {

...
// Default function
export default async function checkIMEI(imei) {
  const jwtToken = document.cookie.split(';').find(cookie => cookie.trim().startsWith('token='));
  console.log("Device IMEI", imei)
    try {
      const verify = await axios.get(`${process.env.REACT_APP_PROVISIONING_PORTAL_API_URL}/provisioning/iws/certus/device/${imei}/detail`,
      {
        headers: {

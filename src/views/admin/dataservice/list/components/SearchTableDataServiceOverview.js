...
function SearchTable2(props) {
  ...
  useEffect(() => {
    setPageSize(Number(filters?.list?.dataservice || 10))
  }, [filters])
        ...
        >
          ...
          {(accountInfo?.isGuardianAdmin || accountInfo?.isAccountManagement) ? (
            <Button
              ...
              _focus={{ bg: "brand.500" }}
              onClick={() => nav('/admin/dataservice/create')}
            >
              Create Data Service
            </Button>
          ) : null}
              ...
              return (
                <Tr {...row.getRowProps()} key={index}>
                  {row.cells.map((cell, index) => {
                    ...
                    if (cell.column.Header === "NAME") {
                    ...
                    } else if (cell.column.Header === "MONTHLY FEE") {
                      data = (
                        <Text color={textColor} fontSize='md' fontWeight='500'>
                          ${cell.value}
                        </Text>
                      );
                    } else if (cell.column.Header === "MARKET") {
                      data = (
                        <Text color={textColor} fontSize='md' fontWeight='500'>
                          {cell.value}
                        </Text>
                      );
                    } else if (cell.column.Header === "INTERNET ACCESS") {
                      if (cell.value === true) {
                        data = (
                    ...
                    } else if (cell.column.Header === "POOLING") {
                      if (cell.value === true) {
                        data = (
                          <Flex justifyContent="left">
                      ...
                      data = (
                        <Text
                          as={Link}
                          to={`${PATH_NAME.DATA_SERVICE_VIEW}?id=${cell.value}`}
                          cursor='pointer'
                          color={brandColor}
          ...
          <Flex align='center'>
            <Select
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('filters', jsonToString(({ ...filters, list: { ...filters?.list, dataservice: e.target.value } })));
              }
              }>

...
import setDevicePoolRule from 'services/alertrules/setPoolAlertRule'
import deleteDeviceAlertRule from 'services/alertrules/deleteDeviceAlertRule'
...
function SearchTable2(props) {
  ...
  const fetchData = async () => {
    try {
      const response = await getAlertRuleBundleList(currentAccountNumber, rowData.relativeThreshold, rowData.thresholds, rowData.usageType);
  ...
  const handleEnable = async (row, enableState) => {
    ...
    try {
      const result = await setDevicePoolRule(
  ...
  const handleDisable = async (row, enableState) => {
    ...
    if (enabledImeis.length === 1) {
      ...
      await disableBundle(row, enableState);
    }
  }

  const disableBundle = async (row, enableState) => {
    try {
  ...
  const handleDialogConfirm = async () => {
    await disableBundle(selectedRow, false);
    handleDialogClose();
  ...
  useEffect(() => {
    setPageSize(Number(filters?.list?.alertbundle || 10))
  }, [filters])
  ...
  return (
    <>
                  ...
                  {row.cells.map((cell, index) => {
                    ...
                    if (cell.column.Header === "PLAN NAME") {
                      data = (
                        ...
                        >
                          {cell.value || row.original.poolID}
                        </Text>
                      );
          ...
          <Flex align='center'>
            <Select
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('filters', jsonToString(({ ...filters, list: { ...filters?.list, alertbundle: e.target.value } })));
              }
              }>
      ...
      >
        <AlertDialogOverlay bg={bgAlertbox}>
          <AlertDialogContent>

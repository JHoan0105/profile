...
// Default function
function SearchTableCreateIMEI(props) {
  const bgAlertbox = useColorModeValue('white', 'guardianDark.500');
  const [isDialogOpen, setIsDialogOpen] = useState(false);                                  // State to manage dialog visibility
  const [filters, setFilters] = useState()                                                  // load user's filter preference
  ...
  const fetchData = async () => {
    try {
        const devices = await getAlertRuleImeiList(currentAccountNumber, false, ruleData.thresholds, 'DEVICE');

        const tmp = devices.map(i => {
          updateRule({ type: RULES.SELECTDEVICE, payload: { imei: i.imei, id: false } })
            return { imei: i.imei, id: false, enabled: false }
          } )

        console.log('Alert rule imei list: ', tmp);
    } catch (error) {
  ...
  const handleAddSelectedDevice = async (rule) => {
    updateRule({ type: RULES.SELECTDEVICE, payload: { imei: rule.imei, id: true } })

  }

  const handleRemoveSelectedDevice = async (rule) => {
    updateRule({ type: RULES.SELECTDEVICE, payload: { imei: rule.imei, id: false } })
  }
  ...
  useEffect(() => {
    setPageSize(Number(filters?.list?.alertcreateimei || 10))
  }, [filters])
      ...
      <Card
            ...
            onClick={() => {
              if (jwtAccountInfo?.isGuardianAdmin || jwtAccountInfo.isAlertManagement) {
                if (ruleData.usageType === 'DEVICE') {
                  console.log({ type: RULES.UPDATE, payload: { name: 'usageType', value: 'POOL' } })
                      ...
                      if (row.original?.id ?? false) {
                        data = (
                          <FaCheckCircle
                            ...
                            onClick={() => handleRemoveSelectedDevice(row.original)} // Call handleClick function on click
                          />
                        );
                      } else {
                        data = (
                          <FaTimesCircle
                            ...
                            size={20}
                            style={{ marginLeft: '15px', cursor: 'pointer' }} // Change cursor to pointer on hover
                            onClick={() => handleAddSelectedDevice(row.original)} // Call handleClick function on click
                          />
                        );
                      }
                    }
        ...
        <Flex
          ...
          <Flex align='center'>
            <Select
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('filters', jsonToString(({ ...filters, list: { ...filters?.list, alertcreateimei: e.target.value } })));
              }
              }>
          ...
          >
            ...
            {ruleData.deviceList.length / pageSize > 15 ? (
              <NumberInput
                max={pageCount - 1}
                min={1}
                w='75px'
...
export default SearchTableCreateIMEI;

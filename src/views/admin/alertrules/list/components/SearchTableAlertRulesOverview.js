...
// Default function
function AlertSearchTable(props) {
  ...
  useEffect(() => {
    setPageSize(Number(filters?.list?.alert || 10))
  }, [filters])
      ...
      <Flex
        ...
        >
          ...
          {(accountInfo?.isGuardianAdmin || accountInfo?.isAlertManagement) ? (
            <Button
              as={Link}
              to={PATH_NAME.ALERT_CREATE}
              state={{ gid: data[0]?.groupId }}
              width='160px'
              variant='brand'
              color='white'
              fontSize='sm'
              ...
              return (
                <Tr {...row.getRowProps()} key={index}>
                  {row.cells.map((cell, index) => {
                    let data = "";
                    if (cell.column.Header === "USAGE TYPE") {
                      data = (
                        <Text color={textColor} fontSize='md' fontWeight='500'>
                          {cell.value}
                        </Text>
                      );
                    } else if (cell.column.Header === "THRESHOLDS") {
                      if (row.original.relativeThreshold) {
                        data = (
                          <Text color={textColor} fontSize='md' fontWeight='500'>
                            {cell.value.split(',').map(val=> val+'%').join(', ')}
                          </Text>
                        );
                      } else {
                        data = (
                          <Text color={textColor} fontSize='md' fontWeight='500'>
                            {cell.value.split(',').map(val => formatBytes(val)).join(', ')}
                          </Text>
                        );
                      }
          ...
          <Flex align='center'>
            <Select
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('filters', jsonToString(({ ...filters, list: { ...filters?.list, alert: e.target.value } })));
              }
              }>
...
export default AlertSearchTable;

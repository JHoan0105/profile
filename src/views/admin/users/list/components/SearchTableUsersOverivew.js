...
function SearchTable2(props) {
  ...
  const checkCircleNewAccount = useColorModeValue("black", "grey");
  ...
  useEffect(() => {
    setPageSize(Number(filters?.list?.user || 10))
  },[filters])
                ...
                <Tr {...row.getRowProps()} key={index}>
                  {row.cells.map((cell, index) => {
                    ...
                    if (cell.column.Header === "NAME") {
                      ...
                      if (row.original.isNewAccount) { //Getting this from a different column
                        data = (
                          <Flex justifyContent="left">
                            <FaCheckCircle color={checkCircleNewAccount} title="New account" size={20} style={{ marginLeft: '15px' }} />
                          </Flex>
                        );
                      } else if (cell.value === true) {
                        data = (
                          <Flex justifyContent="left">
                            <FaCheckCircle color="green" title="User account is not block" size={20} style={{ marginLeft: '15px' }} />
                          </Flex>
                        );
                      } else {
                        data = (
                          <Flex justifyContent="left">
                            <FaTimesCircle color="red" title="User account is block" size={20} style={{ marginLeft: '15px' }} />
                          </Flex>
                        );
                      }
                      ...
                      data = (
                        <Text
                          as={Link}
                          to={PATH_NAME.USER_ADMIN}
                          state={{ admin: (accountInfo?.isGuardianAdmin || accountInfo?.isAccountManagement), userId: row.original.email === accountInfo.email ? undefined:cell.value}}
                          cursor='pointer'
                          color={brandColor}
          ...
          <Flex align='center'>
            <Select
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('filters', jsonToString(({ ...filters, list: { ...filters?.list, user: e.target.value } })));
              }
              }>
